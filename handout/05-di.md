...
`Hamburger` class.  Antoher way of saying this, is that Angular 2 has been
...
`@Inject()` lets Angular 2 know that a _parameter_ needs to be injected.  It can
 be used like so:
...
Checkout [this plunker][plunkBurger1] for a complete `Hamburger` example.

## Injection, Beyond Classes

So far the only types injection has been used for have been classes.  Angular 2
is _not_ limited to injecting classes.  The concept of `providers` was also
...
`useValue`, and `useFactory` are two other examples of `provider` "recipes" that
Angular 2 can use.  For example:
...
The above can be [seen in action here][plunkRandom1], one important note is that
'Random' is in quotes, both in the `provide` function, and in the consumer.  
This is because as a factory we have no `Random` identifier anywhere to access.
...
In [this case][plunkRandom2] the product of `Math.random` is assigned to the
`useValue` property passed to the `provider`.  
...
singletons.  Angular 2 overcomes this limitations by using a tree of injectors.
...
There is a [plunkInjectorTree] with an example. The example consists of a
simple class called `Unique` that generates a unique id each time its 
constructor is run.  The example describes some children components, and
illustrates which componets use which injector.
